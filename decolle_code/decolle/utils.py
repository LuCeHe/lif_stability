...
def parse_args():
    ...
    parser.add_argument('--comments', type=str, default='condIV', help='String to activate extra behaviors')
...
def get_activities(gen_train, decolle_loss, net, opt, epoch, burnin, online_update=True, apply_grad=True):
    ...
    for data_batch, target_batch in tqdm(iter_gen_train, desc='Epoch {}'.format(epoch)):
        ...
        loss_mask = (data_batch.reshape(data_batch.shape[0], data_batch.shape[1], -1).mean(2) > 0.01).unsqueeze(
            2).float()
