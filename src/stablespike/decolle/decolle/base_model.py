...
import warnings
from stablespike.decolle.decolle.utils import get_output_shape, state_detach
...
class LIFLayerPlus(LIFLayer):
    def __init__(self, *args, sg_kwargs={}, **kwargs):
        super(LIFLayerPlus, self).__init__(*args, **kwargs)
        self.sg_function = ConditionedSG(**sg_kwargs)
...
class DECOLLEBase(nn.Module):
    ...
    def process_output(net, data_batch):
        ...
        *data_batch*: batch of inputs, same shape as for data_batch in step()
        '''
        with torch.no_grad():
            from stablespike.decolle.decolle.utils import tonp
            net.init(data_batch)
...
class frDECOLLELoss(object):
    def __init__(self, loss_fn, net, frfrom=None, frto=None, switchep=5, lmbd=0.1, onlyreg=False):
        self.loss_fn = loss_fn
        self.nlayers = len(net)
        self.num_losses = len([l for l in loss_fn if l is not None])
        self.loss_layer = [i for i, l in enumerate(loss_fn) if l is not None]
            ...
            if loss_layer is not None:
                ...
                if self.frfrom is not None:
                    if epoch <= self.switchep:
                        reg = self.lmbd * torch.abs(torch.mean(s[i]) - self.frfrom)
                ...
                if self.frto is not None:
                    if epoch > self.switchep:
                        reg = self.lmbd * torch.abs(torch.mean(s[i]) - self.frto)
