...
def generator_2(data_path, char_or_word, train_val_test, batch_size, maxlen):
    ...
    vocab_path = os.path.join(DATAPATH, 'vocab.json')
    if not os.path.exists(vocab_path):
        path = os.path.join(DATAPATH, 'ptb.train.txt')
    ...
    tokenized_path = os.path.join(DATAPATH, 'tokenized_{}.npy'.format(train_val_test))
    if not os.path.exists(tokenized_path):
        path = os.path.join(DATAPATH, 'ptb.{}.txt'.format(train_val_test.replace('val', 'valid')))
